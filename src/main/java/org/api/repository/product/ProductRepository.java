...
public interface ProductRepository extends BaseRepository<ProductEntity, Integer> {
    ...
    @Query(value = "SELECT * FROM m_product WHERE del_flg = '0' ORDER BY product_id LIMIT 500", nativeQuery = true)
    ...
    @Query(value = "SELECT * " +
            ...
            "name3 LIKE CONCAT('%', :name, '%') OR name4 LIKE CONCAT('%', :name, '%') OR name5 LIKE CONCAT('%', :name, '%'))) LIMIT 500",
            nativeQuery = true)
